...
float det(const Matrix *mat) {
    ...
    int n = mat->rows;
    int *ipiv = (int *)malloc(n * sizeof(int)); // Pivot indices
    float *A_copy = (float *)malloc(n * n * sizeof(float)); // Copy of the original matrix
    float determinant = 1.0;

    if (ipiv == NULL || A_copy == NULL) {
        fprintf(stderr, "Memory allocation failed\n");
        exit(EXIT_FAILURE);
    }

    // Copy the original matrix to A_copy
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            A_copy[i * n + j] = mat->data[i * n + j];
        }
    }

    // Perform LU decomposition
    int info = LAPACKE_sgetrf(LAPACK_ROW_MAJOR, n, n, A_copy, n, ipiv);
    if (info != 0) {
    ...
    for (int i = 0; i < n; i++) {
        determinant *= A_copy[i * n + i]; // Diagonal element
